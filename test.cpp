...
// 测试UserData类的初始化、XML转UserData、UserData转XML
    ...
    pugi::xml_parse_result result_user = doc_user.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\user.xml");
    if (!result_user)
        return -1;
...
// 测试GroupData类的初始化、XML转GroupData、GroupData转XML
    ...
    pugi::xml_parse_result result_group = doc_group.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\group.xml");
    if (!result_group)
        return -1;
...
// 测试RequestData类的初始化、XML转RequestData、RequestData转XML
    ...
    pugi::xml_parse_result result_request = doc_request.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\request.xml");
    if (!result_request)
        return -1;
...
// 测试MessageData类的初始化、XML转MessageData、MessageData转XML
    ...
    pugi::xml_parse_result result_message = doc_message.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\message.xml");
    if (!result_message)
        return -1;
...
// 测试DingDongData类的初始化、XML转DingDongData、DingDongData转XML
    ...
    pugi::xml_parse_result result_data = doc_data.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\data.xml");
    if (!result_data)
        return -1;
...
#include <map>
    ...
    pugi::xml_parse_result result_data_function = doc_data_function.load_file("C:\\Users\\ZombieProcess\\Desktop\\DingDongData\\xml\\data.xml");
    ...
    data_function.deleteGroup(GroupData("216559214"));
    data_function.show();
    ...
    // 添加请求
    RequestData request_1(43252, "123456789", "987654321", "", RequestData::REQUEST_STATUS::agree, 20210612);
    ...
    // 添加消息
    std::pair<MessageID, std::list<MessageData>> mesage_1(std::make_pair(MessageID("123456789", MessageID::ID_TYPE::group), std::list<MessageData>{ MessageData(42122, "123", "456", 20010244, "Ni Hao a") } ));
    std::pair<MessageID, std::list<MessageData>> mesage_2(std::make_pair(MessageID("123456789", MessageID::ID_TYPE::user), std::list<MessageData>{ MessageData(23242, "123", "456", 20010245, "XXX") } ));
    data_function.addMessage(mesage_1);
    data_function.addMessage(mesage_2);
    data_function.addMessage(MessageID("216559214", MessageID::ID_TYPE::group), MessageData(1293, "2248585019", "216559214", 20010713161243, "is so boring too!"));
    data_function.show();
    data_function.deleteMessage(MessageID("123456789", MessageID::ID_TYPE::group), MessageData(42122));
    data_function.show();
    data_function.deleteMessage(MessageID("123456789", MessageID::ID_TYPE::user), MessageData(23242));
    data_function.show();
    data_function.deleteMessage(MessageID("123456789", MessageID::ID_TYPE::user));
    data_function.deleteMessage(MessageID("216559214", MessageID::ID_TYPE::group));
    data_function.show();
